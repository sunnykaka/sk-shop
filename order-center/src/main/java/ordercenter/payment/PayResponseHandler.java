        ...
        if (tradeService.existTradeInfo(tradeInfo.getTradeNo(), tradeInfo.getOuterTradeNo())) {
            ...
            try {
                ...
                if(e.getCause() instanceof org.hibernate.exception.ConstraintViolationException ) {
                    Logger.info("此笔交易遇到并发情况，return notify 同时到达, TradeNo:" + tradeInfo.getTradeNo() + ", 本次通知类型为：" + type, e);
                } else {
                    Logger.error("需要运营人员手动确认：第三方交易成功，但是后续操作异常,交易号为[" + tradeInfo.getTradeNo() + "]-生成交易记录时发生异常", e);
                }
                result.setResult(true);
            }
            ...
            try {
                ...
                Logger.error("需要运营人员手动确认：第三方交易成功，但是后续操作异常,交易号为[" + tradeInfo.getTradeNo() + "]-更新订单状态、扣减库存发生异常", e);
                result.setResult(true);
            }
        }
