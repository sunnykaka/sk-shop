...
import ordercenter.excepiton.OrderNoTransactionalException;
...
public class OrderItem extends TradeItem implements Comparable<OrderItem> {
     ...
     * @param skuStorageService
     * @throws OrderNoTransactionalException
     */
    public void checkAndInit(ProductService productService, ProductPictureService pictureService ,SkuAndStorageService skuStorageService) throws OrderNoTransactionalException {
        // 所有需要 product 赋值的数据全在此处.
        StockKeepingUnit sku = skuStorageService.getStockKeepingUnitById(getSkuId());
        if (sku == null) {
            throw new OrderNoTransactionalException("商品已经不存在!");
        }

        if (sku.getSkuState() == SKUState.REMOVED) {
        ...
        // sku 说明
       // skuExplain = skuStorageService.getSkuPropertyToString(sku); //ldj

        // sku 图片
        ProductPicture productPicture = pictureService.getMainProductPictureByProductIdSKuId(product.getId(), getSkuId());
        // sku 主图的路径
...
//    }
    ...
    private String priceByMoney(long price) {
        return Money.valueOf(price).toString();
    }
